...
import { downloadDevpostCover } from "../lib/scraper/project-images";
...
test("downloads and validates a Devpost CDN cover image", async () => {
  const image = await downloadDevpostCover(
...
test("rejects cover images from unapproved hosts before fetching", async () => {
  ...
  await assert.rejects(
    downloadDevpostCover("https://example.com/cover.png", async () => {
      fetched = true;
